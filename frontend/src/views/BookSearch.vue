...
// Lấy search query từ URL nếu có
searchInput.value = route.query.input?.toString() || ''
...
const searchBooks = async () => {
  ...
  try {
    const response = await axios.get(`${apiUrl}/api/books/search`, {
      params: {
  ...
  <div class="w-full bg-gray-50 min-h-screen">
    ...
    <section class="bg-secondary text-white py-12">
      <div class="container mx-auto px-4">
        <h1 class="text-3xl md:text-4xl font-bold mb-6">{{ t('search.title') }}</h1>
        ...
        <div class="relative max-w-2xl mx-auto">
          <input
            ...
            class="w-full pl-4 pr-12 py-3 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary"
            @keyup.enter="searchBooks"
          >
          <button
            @click="searchBooks"
            class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-primary hover:bg-primary-dark text-white p-2 rounded-lg"
          >
        ...
        <p v-if="searchInput" class="mt-4 text-center">
          {{ t('search.resultsFor') }} "<span class="font-semibold">{{ searchInput }}</span>"
        </p>
      </div>
    </section>
    ...
    <section class="container mx-auto px-4 py-12">
      ...
      <!-- Empty State -->
      <div v-else-if="books.length === 0 && searchInput" class="text-center py-12">
        <h3 class="text-xl font-medium text-gray-700">{{ t('search.noResults') }}</h3>
        <p class="text-gray-500 mt-2">{{ t('search.tryDifferent') }}</p>
      </div>

      <!-- Initial State -->
      <div v-else-if="!searchInput" class="text-center py-12">
        <h3 class="text-xl font-medium text-gray-700">{{ t('search.initialMessage') }}</h3>
      </div>
